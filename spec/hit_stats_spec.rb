...
describe HitStats do
  ...
  it "#to_hash should return hash map of hit stats containing: pass, hit, miss, total and hit/total per class" do
		h = {
			"unset"=> {:pass=>1, :hit=>0, :hit_to_total_ratio=>0.0, :total=>1, :miss=>0},
...
#		@hit_stats.to_hash.should == h
		hs = @hit_stats.to_hash
		hs.keys.should == h.keys

		hs.each_key do |page_class|
			hs[page_class].keys.should == h[page_class].keys
			# round floats to representation length
			hs[page_class].values.sort.map{|v| v.to_s}.should == h[page_class].values.sort.map{|v| v.to_s}
		end
	end
end
